...
if __name__ == '__main__':
    ...
    # 打开数据库连接
    db = pymysql.connect(host='192.168.1.128', user='root', password='123456', db='test')
    # 创建一个游标对象
    cursor = db.cursor()
    table = '''
        ...
        for 分类 in 分类s:
            ...
            for item in data:
                ...
                for tr_index, tr in enumerate(trs):
                    ...
                    字段名称s += '`{}_含量`,`{}_同类排名`,`{}_同类均值`,`{}_含量水平`,'.format(字段名称, 字段名称, 字段名称, 字段名称)
                ...
                sql = 'insert into 食物营养成分查询平台 (大分类名称,分类名称,食物名称,{}) value (%s,%s,%s,{})'.format(字段名称s[:-1], 字段占位符)
                # print('sql', sql)
